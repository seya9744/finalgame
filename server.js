...
// --- 6. BOT COMMAND MENU & LOGIC ---
const bot = new Telegraf(BOT_TOKEN);
bot.use(session());

// Set the "Menu" list commands (The picture you sent)
...
const supportHeader = `ለሚያጋጥማቹ የክፍያ ችግር ወደዚህ መልእክት ይላኩ: \n @sya9744\n@Komodo27 ላይ ፃፉልን።`;

// Command Handlers (Mapped to Menu list)
bot.start(async (ctx) => {
    await User.findOneAndUpdate({ telegramId: ctx.from.id.toString() }, { username: ctx.from.first_name }, { upsert: true });
    ctx.reply(`👋 Welcome to Dil Bingo! Choose an Option below.`, mainKeyboard());
});

bot.command('register', (ctx) => ctx.reply("Please click the button below to register.", contactKey));
bot.command('play', (ctx) => ctx.reply("Launch the game here:", Markup.inlineKeyboard([[Markup.button.webApp("Play Bingo 🎮", MINI_APP_URL)]])));
...
bot.command('balance', async (ctx) => {
    ...
    ctx.reply(`💰 Your Balance: ${u?.balance || 0} Birr`);
});
...
bot.command('instruction', (ctx) => sendInstructions(ctx));
bot.command('support', (ctx) => ctx.reply(`🛠 Support:\n👉 @sya9744\n👉 @komodo27`));

// Helper for Instruction Text (Restored styling)
const sendInstructions = (ctx) => {
    const html = `<b>📘 የቢንጎ ጨዋታ ህጎች</b>\n\n<blockquote><b>🃏 መጫወቻ ካርድ</b>\n\n1. ከ1-300 የመጫወቻ ካርድ ውስጥ አንዱን እንመርጣለን።\n\n2. ቀይ ቀለም የሚያሳዩት በሌላ ተጫዋች መመረጡን ነው።</blockquote>\n\n<blockquote><b>🎮 ጨዋታ</b>\n\n1. ቁጥሮች ከ1 እስከ 75 ይጠራሉ::\n\n2. ካርዶ ላይ ካለ ክሊክ በማረግ ይምረጡ::</blockquote>\n\n<blockquote><b>🏆 አሸናፊ</b>\n\n1. መስመር ሲሞሉ <b>bingo</b> የሚለውን ይንኩ።\n\n2. ተሳስተው ቢጫኑ ከጨዋታ ይታገዳሉ::</blockquote>`;
    ctx.replyWithHTML(html);
};

// --- ACTION & TEXT HANDLERS (Unchanged) ---
bot.action('instructions_trigger', (ctx) => { ctx.answerCbQuery(); sendInstructions(ctx); });
bot.action('dep', (ctx) => { ctx.session = { state: 'WAIT_DEP_AMT' }; ctx.reply("ማስገባት የፈለጉትን የብር መጠን ከ 10 ብር ጀምሮ ያስገቡ።"); });
...
bot.action(/w_meth_(.+)/, (ctx) => {
    ...
    const prompt = (meth === 'CBE' || meth === 'Abyssinia') ? "እባክዎ የአካውንት ቁጥሮን እና ስም ያስገቡ::" : "እባክዎ የስልክ ቁጥሮን እና ስም ያስገቡ::";
...
bot.on('text', async (ctx) => {
    ...
    if (ctx.session?.state === 'WAIT_DEP_AMT') {
        ...
        return ctx.reply(`መጠን: ${amt} ብር\nእባክዎ የክፍያ ዘዴ ይምረጡ:`, Markup.inlineKeyboard([[Markup.button.callback("TELEBIRR", "pay_tele"), Markup.button.callback("CBE", "pay_cbe")],[Markup.button.callback("ABYSSINIA", "pay_aby"), Markup.button.callback("CBE BIRR", "pay_cbebirr")]]));
    }
    if (ctx.session?.state === 'WAIT_W_AMT') {
        const amt = parseInt(text); const u = await User.findOne({ telegramId: uid });
        if (isNaN(amt) || amt < 10 || amt > u.balance) return ctx.reply("❌ የተሳሳተ መጠን።");
