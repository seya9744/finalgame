...
// --- TEXT HANDLER (UPDATED WITHDRAWAL LOGIC) ---
bot.on('text', async (ctx) => {
    ...
    if (ctx.session?.state === 'WAIT_W_AMT') {
        ...
        // Fix 1: Minimum check
        if (isNaN(amt) || amt < 50) {
            return ctx.reply("ዝቅተኛ ማውጣት የሚቻለውየገንዘብ መጠን 50 ብር ነው ።");
        }

        // Fix 2: Insufficient balance check
        if (amt > u.balance) {
            return ctx.reply("ገንዘብ ለማውጣት በቂ Balance የለዎአትም። እባክዎ Deposit በማድረግ ይጫወቱ።");
        }
