...
// --- 5. BOT MENU (RE-BUILT FROM YOUR IMAGE) ---
...
bot.start(async (ctx) => {
    // Proven Version Change: If you see "Option below", the code is updated.
...
// Button Actions
bot.action('reg_prompt', (ctx) => ctx.reply("Please click the phone button at the bottom.", contactKey));
bot.action('bal', async (ctx) => {
    const u = await User.findOne({ telegramId: ctx.from.id.toString() });
    ctx.reply(`💰 Balance: ${u?.balance || 0} Birr`);
});
bot.action('dep', (ctx) => ctx.reply("💰 To deposit, paste your reference number here."));
bot.action('support', (ctx) => ctx.reply("🛠 Support: Contact @YourAdmin"));
bot.action('rules', (ctx) => ctx.reply("📖 Rules: Match 5 in a row to win!"));
bot.action('transfer', (ctx) => ctx.reply("🎁 Transfer coming soon."));
bot.action('withdraw', (ctx) => ctx.reply("🤑 Withdrawal: DM @YourAdmin"));
