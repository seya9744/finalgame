...
function checkServerWin(card, drawnNumbers) {
    ...
    if ([card[0][0], card[0][4], card[4][0], card[4][4]].every(n => drawn.has(n))) return true;
    return false;
}
...
io.on('connection', (socket) => {
    socket.on('register_user', async (data) => {
        try {
            ...
            const u = await User.findOne({ telegramId: tid });
            if (u) {
                if (!u.isRegistered) return socket.emit('error_message', "መጀመሪያ በBot ውስጥ ስልክዎን ያጋሩ (Register)!");
    ...
    socket.on('get_leaderboard', async (period) => {
        let startTime = period === 'Daily' ? new Date().setHours(0,0,0,0) : new Date().setDate(new Date().getDate()-7);
        const top = await GameRecord.aggregate([
            { $match: { date: { $gte: new Date(startTime) } } },
            { $group: { _id: "$telegramId", count: { $sum: 1 } } },
            { $sort: { count: -1 } }, { $limit: 10 },
            { $lookup: { from: "users", localField: "_id", foreignField: "telegramId", as: "u" } },
            { $unwind: "$u" }, { $project: { username: "$u.username", totalPlayed: "$count" } }
        ]);
...
bot.on('contact', async (ctx) => {
    ...
    ctx.reply(`✅ ተመዝግበዋል!${bonusText}`, Markup.removeKeyboard());
...
bot.on('text', async (ctx) => {
    ...
    if (ctx.session?.state === 'WAIT_AMT') {
        ...
        return ctx.reply(`መጠን: ${amt} ብር\nዘዴ ይምረጡ:`, Markup.inlineKeyboard([[Markup.button.callback("TELEBIRR", "pay_tele"), Markup.button.callback("COMMERCIAL BANK", "pay_cbe")]]));
    }
...
bot.action(/w_meth_(.+)/, (ctx) => { const meth = ctx.match[1]; ctx.session.method = meth; ctx.session.state = 'WAIT_W_ID'; ctx.editMessageText(`🏦 ዘዴ: ${meth}\n👤 እባክዎ የስልክ/አካውንት ቁጥሮን ያስገቡ::`); });
...
app.all('/api/incoming-sms', async (req, res) => {
    ...
    if (data) { try { await VerifiedSMS.create({ refNumber: data.ref, amount: data.amount, fullText: text }); } catch (e) {} }
